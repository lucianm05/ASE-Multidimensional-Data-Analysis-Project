...
# Age - filtrăm răspunsurile „Prefer not to say” deoarece nu sunt relevante pentru analiză
#     - aplicăm un label pentru a ușura vizualizarea categoriilor
# AISelect - aplicăm un label pentru a ușura vizualizarea categoriilor (No - nu folosesc, nu doresc; No_Soon - nu folosesc, dar doresc să folosească în curând; Yes- folosesc)
...
# Cateogoria 18-24: aproximativ 10.000 de programatori din această categorie folosesc AI, reprezentând 55%, ceea ce înseamnă că cei care folosesc asemenea instrumente constituie majoritatea
...
pred <- prediction(y_df_m1_pred1_num1,y_df_m1_act1)
pred %>% 
...
df_arbore_train_pred <- predict(df_arbore_final, df_train, type = "prob")[,2]
y_df_m1_pred1_num1 <- ifelse(DTpred_train > 0.5, 1, 0)
y_pred_train_DT <- factor(y_pred_num_train_DT, levels = c(0, 1))
y_act_train <- date_train$Exited
length(DTpred_train)

# Predictii Arbore de Clasificare - TEST set
DTpred_test <- predict(DTfit, date_test, type = "prob")[,2]
y_pred_num_test_DT <- ifelse(DTpred_test > 0.5, 1, 0)
y_pred_test_DT <- factor(y_pred_num_test_DT, levels = c(0, 1))
y_act_test <- date_test$Exited
